...
/* add*/
T_DjiFcSubscriptionSingleBatteryInfo batteryInfo = {0};
static int gs_rcPitch = 0;
static int gs_lastrcPitch = 0;
...
static void *UserFcSubscription_Task(void *arg)
{
    ...
    T_DjiFcSubscriptionFlightStatus lastflight = 3;
    static int getPowerVolDelay = 0; 
    T_DjiVector3f cameraVector3f = {0};
    ...
    while (1) {
        ...
        djiStat = DjiFcSubscription_GetLatestValueOfTopic(DJI_FC_SUBSCRIPTION_TOPIC_STATUS_FLIGHT,
                                                            ...
                                                            &timestamp);
            if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                USER_LOG_ERROR("get value of topic status flight error.");
            }

            if (s_userFcSubscriptionDataShow == true) {
                USER_LOG_INFO("status flight: %d.", flight);
            }
            // DJI_FC_SUBSCRIPTION_FLIGHT_STATUS_STOPED = 0,         无人机在地面上且电机静止
            // DJI_FC_SUBSCRIPTION_FLIGHT_STATUS_ON_GROUND = 1,      无人机在地面上，但电机仍在转动 
            // DJI_FC_SUBSCRIPTION_FLIGHT_STATUS_IN_AIR = 2,         无人机在空中
            
            if(flight != lastflight) {
                lastflight = flight;
                USER_LOG_INFO("status lastflight: %d.", flight);
                /*存入json*/
                // int value = lastflight;
                CziJsonHandler_Open(FILEPATH_JSON_STATE_PSDK, 0);
                printf("(%s LINE-%d)value: %d\n", __FUNCTION__, __LINE__, lastflight);
                CziJsonHandler_WriteInt(JSON_KEY_AIRCRAFT_STATUS, lastflight);
                CziJsonHandler_Close(1);
            }

            /*get aircarft currentVoltage*/
        if (getPowerVolDelay++ >= 20) {
            getPowerVolDelay = 0;
            djiStat = DjiFcSubscription_GetLatestValueOfTopic(DJI_FC_SUBSCRIPTION_TOPIC_BATTERY_SINGLE_INFO_INDEX1,
                                                            (uint8_t *) &batteryInfo,
                                                            sizeof(T_DjiFcSubscriptionSingleBatteryInfo),
                ...
                if (s_userFcSubscriptionDataShow == true) {
                    USER_LOG_INFO("batteryInfo.currentVoltage: %d", batteryInfo.currentVoltage);
                }
                CziPayload_BalancePower(batteryInfo.currentVoltage);
        }

        //Camera Angle processing************************************************************************
        // djiStat = DjiFcSubscription_GetLatestValueOfTopic(DJI_FC_SUBSCRIPTION_TOPIC_GIMBAL_ANGLES,
        //             (uint8_t *) &cameraVector3f,
        //             sizeof(T_DjiVector3f),
        //             &timestamp);
        // // printf("x: %f, y: %f, z: %f\n", cameraVector3f.x, cameraVector3f.y, cameraVector3f.z);
        // if (djiStat != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS){
        //     USER_LOG_ERROR("get gimbal angle error.");
        // }
        // else{
        //     gs_rcPitch = (int)cameraVector3f.x;
        //     gs_rcPitch = 0 - gs_rcPitch;
        //     gs_rcPitch = 100 - gs_rcPitch;
        //     if (gs_rcPitch >= 90)
        //         gs_rcPitch = 100;
        //     if (gs_rcPitch < 0)
        //         gs_rcPitch = 0;
        //     // if(rcPitch > 348) rcPitch = 348;
        //     // else if(rcPitch < -1199) rcPitch = -1199;
        //     if (gs_lastrcPitch != gs_rcPitch) {
        //         printf("rcPitch is %d\n", gs_rcPitch);
        //         gs_lastrcPitch = gs_rcPitch;
        //         CziPayload_SetGimbalAngle(gs_rcPitch);
        //     }
