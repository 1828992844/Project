...
T_DjiReturnCode CziUpgrade_RegUpgradePlatformOpt(const T_DjiTestUpgradePlatformOpt *upgradePlatformOpt)
{
...
T_DjiReturnCode CziUpgrade_RebootSystem(void)
{
    return s_upgradePlatformOpt.rebootSystem();
}

T_DjiReturnCode CziUpgrade_CleanUpgradeProgramFileStoreArea(void)
{
    return s_upgradePlatformOpt.cleanUpgradeProgramFileStoreArea();
}

T_DjiReturnCode CziUpgrade_CreateUpgradeProgramFile(const T_DjiUpgradeFileInfo *fileInfo)
{
    return s_upgradePlatformOpt.createUpgradeProgramFile(fileInfo);
}

T_DjiReturnCode CziUpgrade_WriteUpgradeProgramFile(uint32_t offset, const uint8_t *data, uint16_t dataLen)
{
    return s_upgradePlatformOpt.writeUpgradeProgramFile(offset, data, dataLen);
}

T_DjiReturnCode CziUpgrade_ReadUpgradeProgramFile(uint32_t offset, uint16_t readDataLen, uint8_t *data,
                                                 uint16_t *realLen)
{
    return s_upgradePlatformOpt.readUpgradeProgramFile(offset, readDataLen, data, realLen);
}

T_DjiReturnCode CziUpgrade_CloseUpgradeProgramFile(void)
{
    return s_upgradePlatformOpt.closeUpgradeProgramFile();
}

T_DjiReturnCode CziUpgrade_ReplaceOldProgram(void)
{
    return s_upgradePlatformOpt.replaceOldProgram();
}

T_DjiReturnCode CziUpgrade_SetUpgradeRebootState(const T_DjiUpgradeEndInfo *upgradeEndInfo)
{
    return s_upgradePlatformOpt.setUpgradeRebootState(upgradeEndInfo);
}

T_DjiReturnCode CziUpgrade_GetUpgradeRebootState(bool *isUpgradeReboot, T_DjiUpgradeEndInfo *upgradeEndInfo)
{
    return s_upgradePlatformOpt.getUpgradeRebootState(isUpgradeReboot, upgradeEndInfo);
}

T_DjiReturnCode CziUpgrade_CleanUpgradeRebootState(void)
{
