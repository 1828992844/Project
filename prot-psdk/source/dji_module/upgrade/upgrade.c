...
#include "upgrade/upgrade_common_file_transfer.h"
#include "upgrade/upgrade_platform_opt.h"
...
static T_DjiReturnCode CziUpgrade_EnterUpgradeMode(uint16_t *waitTime);
static T_DjiReturnCode CziUpgrade_CheckFirmware(void);
static T_DjiReturnCode CziUpgrade_StartUpgrade(void);
static T_DjiReturnCode CziUpgrade_FinishUpgrade(void);
static void *CziUpgrade_UpgradeProcessTask(void *arg);
static void *CziUpgrade_EnterUpgradeModeProcessTask(void *arg);

/* Exported functions definition ---------------------------------------------*/
T_DjiReturnCode CziUpgrade_StartService(T_DjiFirmwareVersion firmwareVersion)
{
    T_DjiTestUpgradePlatformOpt upgradePlatformOpt = {
    ...
    T_DjiUpgradeHandler s_upgradeHandler = {
        .EnterUpgradeMode = CziUpgrade_EnterUpgradeMode,
        .CheckFirmware = CziUpgrade_CheckFirmware,
        .StartUpgrade = CziUpgrade_StartUpgrade,
        .FinishUpgrade = CziUpgrade_FinishUpgrade
    };
    returnCode = DjiCore_SetFirmwareVersion(testUpgradeConfig.firmwareVersion);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("set firmware version error");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    returnCode = CziUpgrade_RegUpgradePlatformOpt(&upgradePlatformOpt);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    returnCode = CziUpgrade_GetUpgradeRebootState(&isUpgradeReboot, &upgradeEndInfo);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Get upgrade reboot state error");
        isUpgradeReboot = false;
    }

    returnCode = CziUpgrade_CleanUpgradeRebootState();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    if (osalHandler->TaskCreate("upgrade_task", CziUpgrade_UpgradeProcessTask, UPGRADE_TASK_STACK_SIZE, NULL,
                                &s_upgradeProcessThread) != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Dji upgrade test task create error.");
        return DJI_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }

    if (osalHandler->TaskCreate("enter_upgrade_mode_task", CziUpgrade_EnterUpgradeModeProcessTask, UPGRADE_TASK_STACK_SIZE,
...
static T_DjiReturnCode CziUpgrade_EnterUpgradeMode(uint16_t *waitTime)
{
    ...
    *waitTime = DJI_TEST_ENTER_UPGRADE_WAIT_TIME;
    // enable is need enter upgrade mode process, the process is in  CziUpgrade_EnterUpgradeModeProcessTask
...
static void *CziUpgrade_EnterUpgradeModeProcessTask(void *arg)
{
    ...
    while (1) {
        if (s_isNeedEnterUpgradeModeProcess) {
            ...
            returnCode = CziUpgrade_CleanUpgradeProgramFileStoreArea();
            if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    while (1) {
        ...
        if (tempUpgradeState.upgradeStage == DJI_UPGRADE_STAGE_ONGOING) {
            if (s_isNeedReplaceProgramBeforeReboot) {
                ...
                returnCode = CziUpgrade_ReplaceOldProgram();
                if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            ...
            returnCode = CziUpgrade_SetUpgradeRebootState(&upgradeEndInfo);
            if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
            ...
            returnCode = CziUpgrade_RebootSystem();
            if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
