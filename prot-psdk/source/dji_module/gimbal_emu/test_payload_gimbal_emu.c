...
#include "../../czi_payload/czi_payload.h"
...
static unsigned int gl_task_count = 0;//用于转换ms与s
static unsigned char gl_Cali_Flag = 0;//用于标记校准
static unsigned char gl_cali_count = 0;
static unsigned char wait_Gl_Ack = 0;//用于等待GL校准开始
static unsigned char wait_gl_ack_count = 0;

uint8_t canSetAngle = 1;
// extern T_megaphone mega;
...
T_DjiReturnCode DjiTest_GimbalStartService(void)
{
    ...
    if (osalHandler->TaskCreate("user_gimbal_task", UserGimbal_Task,
        ...
        USER_LOG_INFO("user gimbal task create error");
        return DJI_ERROR_SYSTEM_MODULE_CODE_UNKNOWN;
    }
...
void Gimbal_SetAngel(void)
{
    if(canSetAngle){
        printf("########### pc:%d\n", s_attitudeInformation.attitude.pitch);
        G_angle = (s_attitudeInformation.attitude.pitch+900)*0.1;	
        	
        printf("!!!!!!!!!!!!!!!!!!!!!!!G_angle : %d\n",G_angle);
        CziPayload_SetGimbalAngle(G_angle);
        if(G_angle >= 0 && G_angle <= 90)
            mega_setAngle(G_angle); 
        else if(G_angle < 0)
	    mega_setAngle(0);
	else if(G_angle > 90)
	    mega_setAngle(90); 
                                     ...
                                     T_DjiAttitude3d rotationValue)
{
    ...
    switch (rotationMode) {
        ...
        case DJI_GIMBAL_ROTATION_MODE_ABSOLUTE_ANGLE:
            printf("gimbal absolute rotate angle: pitch %d, roll %d, yaw %d.", rotationValue.pitch,
                          rotationValue.roll, rotationValue.yaw);
            ...
            //         Gimbal_SetAngel();
            //         break;
            //     case SET_ANGLE_FREQ*2:
            //         Gimbal_SetAngel();
            //         break;
            //     case SET_ANGLE_FREQ*3:
            //         Gimbal_SetAngel();
            //         break;
            //     case SET_ANGLE_FREQ*4:
            //         Gimbal_SetAngel();
            //         break;
            //     case SET_ANGLE_FREQ*5:
            //         Gimbal_SetAngel();
            //         break;			
            //     case SET_ANGLE_FREQ*6:
            //         Gimbal_SetAngel();			
            //         break;							
            //     case SET_ANGLE_FREQ*10:
            //         Gimbal_SetAngel();
            //         G_delay = SET_ANGLE_FREQ*6;
            //         break;
            // }
            // G_delay++;
            // if (s_rotatingFlag == true) {
...
unsigned char Get_Cali_Flag(void){
    return gl_Cali_Flag;
}

T_DjiReturnCode Set_Cali_Flag(unsigned char value){
    gl_Cali_Flag = value;
    return DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS;
}

unsigned char Get_Wait_Gl_Ack(void){
    return wait_Gl_Ack;
}

T_DjiReturnCode Set_Wait_Gl_Ack(unsigned char value){
    wait_Gl_Ack = value;
    return DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS;
}

// static int mmega_statusPlay(char stat)
// {
//     char text[2][3][64]={
//         { "system starup", "server connecting", "server connected", "user login", "system upgrade","power off", "system error" },
//         {{0xE4,0xBA,0x91,0xE5,0x8F,0xB0,0xE6,0xA0,0xA1,0xE5,0x87,0x86,0xE5,0xBC,0x80,0xE5,0xA7,0x8B},//云台校准开始，
//         {0xE6,0xA0,0xA1,0xE5,0x87,0x86,0xE4,0xB8,0xAD,0xE8,0xAF,0xB7,0xE7,0xA8,0x8D,0xE5,0x80,0x99},//校准中请稍候，
//         {0xE4,0xBA,0x91,0xE5,0x8F,0xB0,0xE6,0xA0,0xA1,0xE5,0x87,0x86,0xE5,0xAE,0x8C,0xE6,0x88,0x90},//云台校准完成
//         }
//     };
//     media_setTts(CHN_SYSTEM, mega.ttsSelect, text[1][(stat<5)?stat:5]);
//     return 0;
// }
