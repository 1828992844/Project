...
static T_DjiReturnCode GetMediaFileDir(char *dirPath)
{
    ...
    int offset = strlen(dirPath);
    snprintf((char *)&dirPath[offset], DJI_FILE_PATH_SIZE_MAX - offset, "%s", "smedia_file");
...
static void *UserCameraMedia_SendVideoTask(void *arg)
{
    ...
    if (s_isMediaFileDirPathConfigured == true) {
        ...
        snprintf(tempPath, DJI_FILE_PATH_SIZE_MAX, "%s", s_mediaFileDirPath);
        int offset = strlen(tempPath);
        snprintf((char *)&tempPath[offset], DJI_FILE_PATH_SIZE_MAX - offset, "%s", "PSDK_0005.h264");
    } else {
        // snprintf(tempPath, DJI_FILE_PATH_SIZE_MAX, "%smedia_file/PSDK_0005.h264", curFileDirPath);
        snprintf(tempPath, DJI_FILE_PATH_SIZE_MAX, "%s", curFileDirPath);
        int offset = strlen(tempPath);
        snprintf((char *)&tempPath[offset], DJI_FILE_PATH_SIZE_MAX - offset, "%s", "media_file/PSDK_0005.h264");
    }
