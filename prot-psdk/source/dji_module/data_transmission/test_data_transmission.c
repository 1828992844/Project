...
{
    printf("s_aircraftInfoBaseInfo.mountPosition is %d\n", s_aircraftInfoBaseInfo.mountPosition);
...
// static void DataTransmission_CalcStreamTime(unsigned long num)
// {
//     static float oldTime_ms;
//     static float totalTime;
//     struct timeval tm_now;
//     gettimeofday(&tm_now,NULL);
//     struct tm *nowTime;
//     nowTime = localtime(&tm_now.tv_sec) ;
//     float nowTime_ms = (nowTime->tm_min*60+nowTime->tm_sec)*1000 +tm_now.tv_usec/1000;

//     if (num) {
//         float detaTime = nowTime_ms - oldTime_ms;
//         totalTime += detaTime;
//         float avgDetaTime = totalTime/num;
//         printf("****stream (%ld, total: %.3fms, deta: %.3fms, avg: %.3fms)****\n", num, totalTime, detaTime, avgDetaTime);
//     }
//     else {
//         oldTime_ms = 0.0;
//         totalTime = 0.0;
...
static T_DjiReturnCode ReceiveDataFromMobile(const uint8_t *data, uint16_t len)
{
    ...
    if(data[3] == 0x2A) {
        // DataTransmission_CalcStreamTime(s_num);
        CziTransmission_SendDataByMailbox(data, len);
        // s_num++;
        return DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS;
    }
    ...
    printData[len] = '\0';
    CziTransmission_SetQueueDataById(0x03, data, len);
