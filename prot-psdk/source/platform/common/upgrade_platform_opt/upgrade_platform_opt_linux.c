...
T_DjiReturnCode DjiUpgradePlatformLinux_RebootSystem(void)
{
    ...
    CziJsonHandler_Open(FILEPATH_CONFIG_UPDATE_JSON, 0);
    CziJsonHandler_WriteString(JSON_KEY_UPDATE, "ON");
    CziJsonHandler_Close(1);
    // return DjiTest_RunSystemCmd("reboot -h now");
    return DjiTest_RunSystemCmd("reboot -f");
}
...
T_DjiReturnCode DjiUpgradePlatformLinux_CleanUpgradeProgramFileStoreArea(void)
{
    ...
    snprintf(cmdBuffer, DJI_TEST_CMD_CALL_MAX_LEN, "rm %s*_V*.*.*.bin", DJI_TEST_UPGRADE_FILE_DIR);
