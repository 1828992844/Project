...
/* Private constants ---------------------------------------------------------*/
#define DJI_LOG_PATH                    "/czzn/clog/DJI"
#define DJI_LOG_INDEX_FILE_NAME         "/czzn/clog/index"
#define DJI_LOG_FOLDER_NAME             "/czzn/clog/DJI"
...
T_DjiReturnCode CziMultiTask_ApplicationStart(void *args)
{
    ...
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("Core init error  %d\n",returnCode);
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
    ...
    returnCode = DjiCore_SetAlias(CZI_PAYLOAD_MODEL);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
    ...
    returnCode = CziPayload_ApplicationStart();
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("start czi payload application error");
        return DJI_ERROR_SYSTEM_MODULE_CODE_SYSTEM_ERROR;
    }
...
//         // if (aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_SKYPORT_V2 ||
//         //     aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_NONE) {
//             if (DjiTest_CameraEmuMediaStartService() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
//                 USER_LOG_ERROR("psdk gimbal init error");
//             }
//         // }
// #endif
...
//     if (DjiTest_CameraEmuBaseStartService() != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
//         USER_LOG_ERROR("camera emu common init error");
//     }
// #endif
...
// #endif
    ...
    returnCode = CziUpgrade_StartService(firmwareVersion);
    if (returnCode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
        USER_LOG_ERROR("psdk upgrade init error");
    }
...
{
    DjiUser_NormalExitHandler(signalNum);

    CziPayload_NormalExitHandler(signalNum);
...
static T_DjiReturnCode DjiUser_FillInUserInfo(T_DjiUserInfo *userInfo)
{
    ...
    if (!strcmp(USER_APP_NAME, "your_app_name") ||
        ...
        USER_LOG_ERROR(
            "Please fill in correct user information to 'platform/linux/manifold2/application/dji_sdk_app_info.h' file.");
        sleep(1);
        return DJI_ERROR_SYSTEM_MODULE_CODE_INVALID_PARAMETER;
    }
